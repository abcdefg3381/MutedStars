...
public class StartSuperStarDB {

	public static void main(String[] args) throws IOException {
		new StartSuperStarDB();
	}
	...
	private Image createImage(String path, String description) {
		URL imageURL = StartSuperStarDB.class.getResource(path);
	...
	private Menu getStartMenuItem() {
		if (startMenuItem == null) {
			...
			for (final String name : selectDB()) {
				MenuItem item = new MenuItem(name.substring(0, name.lastIndexOf(".properties")));
				item.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent arg0) {
						stopDB();
	...
	private void startDB(String dbname) {
		...
		hsqldbServer.setDatabasePath(0, "./resources/dbmodel/" + dbname
				...
				+ "hsqldb.cache_file_scale=8;" + "hsqldb.log_size=1000");
		hsqldbServer.setDatabaseName(0, "SuperStar");
